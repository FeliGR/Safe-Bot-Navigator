...
class BasicQLearningAgent:
    # History types for plotting
    TRAIN_HISTORY = 'train'
    GREEDY_HISTORY = 'greedy'
    BOTH_HISTORIES = 'both'
    VALID_HISTORY_TYPES = {TRAIN_HISTORY, GREEDY_HISTORY, BOTH_HISTORIES}

    def __init__(self, state_size, n_actions, learning_rate=0.1, gamma=0.95, epsilon=1.0, epsilon_min=0.01, epsilon_decay=0.001):
    ...
    def get_action(self, state):
        ...
        # Get all actions with maximum Q-value for the state
        options = np.argwhere(self.q_table[state] == np.max(self.q_table[state])).flatten()
    ...
    def update(self, state, action, reward, next_state):
        ...
        self.q_table[state][action] = current_q + self.learning_rate * (reward + self.gamma * future_q - current_q)
    
    def decay_epsilon(self):
        """Decay epsilon after an episode by subtracting epsilon_decay"""
        self.epsilon = max(self.epsilon_min, self.epsilon - self.epsilon_decay)
    
    def train(self, env, episodes=10000, max_steps=1000, render_freq=100, render_mode=None, render_delay=0.1):
    ...
    def run_greedy(self, env, episodes=1, max_steps=1000, render_mode="human", render_delay=0.1):
    ...
    def plot_history(self, history_type=BOTH_HISTORIES, save_path=None):
        ...
        histories = []
        if history_type in [self.TRAIN_HISTORY, self.BOTH_HISTORIES] and self.train_history:
            histories.append(('Training', self.train_history))
        if history_type in [self.GREEDY_HISTORY, self.BOTH_HISTORIES] and self.greedy_history:
        ...
        n_metrics = sum(len([k for k in h[1].keys() if k != 'episodes']) for h in histories)
