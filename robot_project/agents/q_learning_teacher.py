...
class QLearningAgentTeacher(BasicQLearningAgent):
    ...
    def get_action(self, state, env=None):
        """
        Get action based on current state and Q-values difference.
        If Q-values difference is less than threshold, use planifier.
        """
        state_q_values = self.q_table[state]
        q_diff = np.max(state_q_values) - np.min(state_q_values)

        if q_diff < self.q_threshold and env is not None:
            print("Using planifier...")
