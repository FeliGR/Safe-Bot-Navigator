...
class GridEnvironment:
    ...
    def __init__(self, size=6, obstacle_prob=0.2, trap_prob=0.5, trap_danger=0.9, rewards=None):
        ...
        self.risk_assessment = RiskAssessment(self.grid)
        ...
        if (0 <= self.robot_pos[0] < self.size and 
            ...
            if cell_type == self.OBSTACLE:  # Collision with obstacle
                ...
                if np.random.random() < self.trap_danger:
                    ...
                    self.text_color = self.RED
                    self._wait_for_message()
                    self.risk_assessment.increase_risk(self.robot_pos)
    ...
    def find_shortest_path(self, allow_traps=False):
        ...
        frontier = [(manhattan_distance(start, goal), 0, start, [])]  # (f_score, g_score, position, path)
        visited = set()
        ...
        while frontier:
            ...
            for next_pos, action in get_neighbors(current):
                if next_pos not in visited:
                    ...
                    insert_idx = 0
                    while (insert_idx < len(frontier) and 
                           frontier[insert_idx][0] < new_f):
                        insert_idx += 1
