...
def create_policy_visualization(agent, env, run_name, save_path=None):
    ...
    for i in range(size):
        for j in range(size):
            ...
            if (
                cell_type != GridEnvironment.OBSTACLE
                and cell_type != GridEnvironment.TARGET
            ):
                ...
                vertical_actions = sorted(
                    [
                        a
                        for a in best_actions
                        if a in [GridEnvironment.MOVE_UP, GridEnvironment.MOVE_DOWN]
                    ]
                )
                horizontal_actions = sorted(
                    [
                        a
                        for a in best_actions
                        if a in [GridEnvironment.MOVE_LEFT, GridEnvironment.MOVE_RIGHT]
                    ]
                )
                ...
                for idx, action in enumerate(vertical_actions):
                    ...
                    arrow_color = (
                        "white" if cell_type == GridEnvironment.TRAP else "blue"
                    )
                    ax.arrow(
                        center_x,  # Start from center x
            ...
            for j in range(size):
                ...
                if (
                    ...
                    for action, q_value in enumerate(q_values):
                        ...
                        ax.text(
                            j + padding + 0.5,
                            size - 1 - i + padding + 0.3 + 0.1 * action,
                            f"{action_name}: {q_value:.2f}",
                            fontsize=8,
                            ha="center",
                            va="center",
                            color="black",
    ...
    legend_elements = [
        ...
        plt.Rectangle(
            (0, 0), 1, 1, facecolor="gray", edgecolor="black", label="Obstacle"
        ),
        plt.Rectangle((0, 0), 1, 1, facecolor="red", edgecolor="black", label="Trap"),
        plt.Rectangle(
            (0, 0), 1, 1, facecolor="green", edgecolor="black", label="Target"
        ),
        plt.Line2D(
            [0], [0], color="blue", marker=">", linestyle="-", label="Optimal Action"
        ),
    ]
