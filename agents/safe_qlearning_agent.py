...
class SafeQLearningAgent(BasicQLearningAgent):
    ...
    def is_safe_state(self, state, env):
        ...
        return (
            trap_distance >= self.trap_threshold
    ...
    def get_action(self, state, env=None):
        ...
        if self.safety_threshold is not None and safety_factor < self.safety_threshold:
            if self.interaction_type == self.UNIFORM:
                ...
                if (
                    q_diff < safety_factor
                ):  # Use Q-value uncertainty as additional criterion
    ...
    def plot_history(
        self, history_type=BasicQLearningAgent.BOTH_HISTORIES, save_path=None
    ):
        ...
        for i, (history, label) in enumerate(histories):
            ...
            if "trap_violations" in history:
                ...
                plt.plot(
                    history["episodes"],
                    history["trap_violations"],
                    label="Trap Violations",
                )
            ...
            if "obstacle_violations" in history:
                ...
                plt.plot(
                    history["episodes"],
                    history["obstacle_violations"],
                    label="Obstacle Violations",
                )
            ...
            if "planifier_uses" in history:
                ...
                plt.plot(
                    history["episodes"],
                    history["planifier_uses"],
                    label="Planifier Uses",
                )
